      ...
      <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
        ...
        <div className='flex-1.5 w-full flex flex-row justify-between flex-wrap md:mt-0 mt-10'>
          {footerLinks.map((link) => (
            <div key={link.key} className='flex-col flex ss:my-4 min-w-[150px]'>
              <h4 className='font-poppins font-medium text-[18px] leading-[27px] text-white'>{link.title}</h4>
              <ul className='list-none mt-4'>
                {link.links.map((str, index) => (
                  <li key={str.name} className={`font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${index !== link.links.length -1 ? 'mb-4' : 'mb-0'}`}>
                    {str.name}
                  </li>
                ))}
              </ul>
